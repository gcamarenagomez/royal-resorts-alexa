...
exports.SearchBags = (slots, session, response) => {
    ...
    response.ask("OK, can I have your Reservation Code?");
}

exports.AnswerPNR = (slots, session, response) => {
    if(session.attributes.stage === 'ask_pnr'){
        let pnr = slots.pnr.value;
        session.attributes.pnr = pnr;
        salesforce.findCases({pnr: session.attributes.pnr})
        .then(cases => {
            if(cases && cases.length>0){
                let text = `OK, here is what I found for Reservation ${session.attributes.pnr}: `;
                cases.forEach(c => {
                    text += `Case No. ${c.get('CaseNumber')}, Subject ${c.get('Subject')} has a status of: ${c.get('Status')}`;
                });
                response.say(text);
            }
            else{
                response.say(`Sorry, I did not find any cases for Reservation ${session.attributes.pnr}`);
            }
        })
        .catch((err)=>{
            console.error(err);
